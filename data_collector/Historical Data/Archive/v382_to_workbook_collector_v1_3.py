...
class WorkbookDataCollector:
    ...
    def collect_yahoo_finance_historical(self):
        ...
        # ACWX - 90 days for 30D/90D MA calculation
        try:
            ...
            if not data.empty:
                close_data = data['Close'].tail(90)
                values = [round(float(v), 2) for v in close_data]
                dates = [d.strftime('%Y-%m-%d') for d in close_data.index]
                
                self.collected_data['acwx'] = {
                    'values': values,
                    'dates': dates,
                    'source': 'Yahoo Finance API',
                    'collection_status': 'success'
                }
                self.logger.info(f"  ACWX: {len(values)} days collected")
            else:
                self.collected_data['acwx'] = {'collection_status': 'failed', 'reason': 'No data returned'}
                
        except Exception as e:
            self.collected_data['acwx'] = {'collection_status': 'failed', 'reason': str(e)}
            self.logger.error(f"  ACWX failed: {e}")
        
        # VEU - for S&P vs World calculation
        try:
            ticker = yf.Ticker("VEU")
            start_date = datetime.now() - timedelta(days=400)  # Need more history for 12M returns
            data = ticker.history(start=start_date)
            ...
            if not data.empty:
                ...
                self.collected_data['veu'] = {
                    'values': values,
                    'dates': dates,
                    'source': 'Yahoo Finance API',
                    'collection_status': 'success'
                }
                self.logger.info(f"  VEU: {len(values)} days collected")
            else:
                self.collected_data['veu'] = {'collection_status': 'failed', 'reason': 'No data returned'}
                
        except Exception as e:
            self.collected_data['veu'] = {'collection_status': 'failed', 'reason': str(e)}
            self.logger.error(f"  VEU failed: {e}")
        
        # Treasury 10Y for Risk Premium
        try:
            ticker = yf.Ticker("^TNX")
            start_date = datetime.now() - timedelta(days=200)  # 6 months + buffer
            data = ticker.history(start=start_date)
            
            if not data.empty:
                close_data = data['Close'].tail(150)
                values = [round(float(v), 2) for v in close_data]
                dates = [d.strftime('%Y-%m-%d') for d in close_data.index]
                
                self.collected_data['treasury_10y'] = {
                    'values': values,
                    'dates': dates,
                    'source': 'Yahoo Finance API',
                    'collection_status': 'success'
                }
                self.logger.info(f"  Treasury 10Y: {len(values)} days collected")
            else:
                self.collected_data['treasury_10y'] = {'collection_status': 'failed', 'reason': 'No data returned'}
                
        except Exception as e:
            self.collected_data['treasury_10y'] = {'collection_status': 'failed', 'reason': str(e)}
            self.logger.error(f"  Treasury 10Y failed: {e}")
    ...
    def collect_what_we_can(self):
        ...
        # Calculate ACWX/SPY ratio if both collected
        if ('acwx' in self.collected_data and self.collected_data['acwx']['collection_status'] == 'success' and
            'spy' in self.collected_data and self.collected_data['spy']['collection_status'] == 'success'):
            
            self.logger.info("Calculating ACWX/SPY ratio...")
            acwx_values = self.collected_data['acwx']['values']
            spy_values = self.collected_data['spy']['values']
            
            # Match dates between ACWX (90 days) and SPY (300 days) - use last 90 days of SPY
            spy_matched = spy_values[-len(acwx_values):] if len(spy_values) >= len(acwx_values) else spy_values
            
            ratio_values = []
            for i in range(min(len(acwx_values), len(spy_matched))):
                if acwx_values[i] and spy_matched[i]:
                    ratio_values.append(round(acwx_values[i] / spy_matched[i], 4))
                else:
                    ratio_values.append('')
        ...
        # Calculate S&P vs World (SPY vs VEU) performance difference
        if ('spy' in self.collected_data and self.collected_data['spy']['collection_status'] == 'success' and
            'veu' in self.collected_data and self.collected_data['veu']['collection_status'] == 'success'):
            
            self.logger.info("Calculating S&P vs World (SPY vs VEU)...")
            spy_values = self.collected_data['spy']['values']
            veu_values = self.collected_data['veu']['values']
            
            # Calculate 6-month relative performance (26 weeks ≈ 6 months)
            performance_diff = []
            min_length = min(len(spy_values), len(veu_values))
            
            for i in range(130, min_length):  # Start after 130 days (≈6 months) for lookback
            ...
            # Use SPY dates for the matched period
        ...
        # Calculate Risk Premium (Treasury yield vs earnings yield)
        if 'treasury_10y' in self.collected_data and self.collected_data['treasury_10y']['collection_status'] == 'success':
            self.logger.info("Calculating Equity Risk Premium...")
            treasury_values = self.collected_data['treasury_10y']['values']
            
            # Estimate earnings yield (inverse of P/E ratio, roughly 4.5% for current market)
            estimated_earnings_yield = 4.5  # Rough estimate - can be refined
            ...
            self.collected_data['risk_premium'] = {
                ...
                'collection_status': 'success',
                'note': 'Earnings yield estimated at 4.5% - can be refined with actual S&P 500 data'
            }
    ...
    def output_to_workbook(self, workbook_filename=None):
        ...
        if 'qqq_spy_ratio' in self.collected_data:
            ...
            ratio_df_data = {
                ...
                'QQQ_Close': self.collected_data['qqq']['values'][:len(ratio_data['dates'])] if 'qqq' in self.collected_data else [''] * len(ratio_data['dates']),
                'SPY_Close': self.collected_data['spy']['values'][:len(ratio_data['dates'])] if 'spy' in self.collected_data else [''] * len(ratio_data['dates']),
        ...
        # ACWX/SPY ratio tab if calculated
        if 'acwx_spy_ratio' in self.collected_data:
            ratio_data = self.collected_data['acwx_spy_ratio']
            
            ratio_df_data = {
                'Date': ratio_data['dates'],
                'ACWX_SPY_Ratio': ratio_data['values'],
                'ACWX_Close': self.collected_data['acwx']['values'][:len(ratio_data['dates'])] if 'acwx' in self.collected_data else [''] * len(ratio_data['dates']),
                'SPY_Close': self.collected_data['spy']['values'][-len(ratio_data['dates']):] if 'spy' in self.collected_data else [''] * len(ratio_data['dates']),
                'Notes': ['Auto-calculated'] * len(ratio_data['values'])
            }
            ...
            instructions = pd.DataFrame({
                'Date': [
                    'ACWX/SPY RATIO - Daily Data',
                    'AUTO-CALCULATED: Ratios below were automatically calculated',
                    'Source: Yahoo Finance ACWX and SPY prices',
                    'Frequency: Daily',
                    'Time Period: 90 days for 30D/90D MA calculation',
                    'Status: Complete ratio time series',
                    'Manual additions: None needed for this indicator',
                    '--- AUTO-CALCULATED DATA BELOW ---'
                ],
            ...
            tabs['ACWX_SPY_Ratio'] = pd.concat([instructions, pd.DataFrame(ratio_df_data)], ignore_index=True)
        
        # S&P vs World tab if calculated
        if 'sp_vs_world' in self.collected_data:
            perf_data = self.collected_data['sp_vs_world']
            
            perf_df_data = {
                'Date': perf_data['dates'],
                'SPY_vs_VEU_Diff': perf_data['values'],
                'SPY_6M_Return': [''] * len(perf_data['values']),  # Could calculate separately
                'VEU_6M_Return': [''] * len(perf_data['values']),  # Could calculate separately
                'Notes': ['Auto-calculated 6M relative performance'] * len(perf_data['values'])
            }
            
            instructions = pd.DataFrame({
                'Date': [
                    'S&P vs WORLD (SPY vs VEU) - Performance Difference',
                    'AUTO-CALCULATED: Performance differences below were calculated',
                    'Source: Yahoo Finance SPY vs VEU (6-month relative)',
                    'Frequency: Daily',
                    'Methodology: 6M SPY return minus 6M VEU return',
                    'Proxy: VEU represents developed markets ex-US',
                    'Status: Ready for -2% threshold analysis',
                    '--- AUTO-CALCULATED DATA BELOW ---'
                ],
                'SPY_vs_VEU_Diff': [''] * 8,
                'SPY_6M_Return': [''] * 8,
            ...
            tabs['SP_vs_World'] = pd.concat([instructions, pd.DataFrame(perf_df_data)], ignore_index=True)
        
        # Risk Premium tab if calculated
        if 'risk_premium' in self.collected_data:
            rp_data = self.collected_data['risk_premium']
            
            rp_df_data = {
                'Date': rp_data['dates'],
                'Risk_Premium': rp_data['values'],
                'Treasury_10Y': self.collected_data['treasury_10y']['values'][:len(rp_data['dates'])] if 'treasury_10y' in self.collected_data else [''] * len(rp_data['dates']),
                'Earnings_Yield': [4.5] * len(rp_data['values']),  # Estimated constant
                'Notes': ['Auto-calculated with estimated earnings yield'] * len(rp_data['values'])
            }
            
            instructions = pd.DataFrame({
                'Date': [
                    'EQUITY RISK PREMIUM - Daily Data',
                    'AUTO-CALCULATED: Risk premiums below were calculated',
                    'Source: Treasury 10Y (Yahoo) + Estimated Earnings Yield',
                    'Frequency: Daily',
                    'Formula: Earnings Yield (4.5% est.) - Treasury 10Y',
                    'Improvement: Replace 4.5% with actual S&P 500 earnings yield',
                    'Status: Functional but can be refined',
                    '--- AUTO-CALCULATED DATA BELOW ---'
                ],
                'Risk_Premium': [''] * 8,
                'Treasury_10Y': [''] * 8,
            ...
            tabs['Risk_Premium'] = pd.concat([instructions, pd.DataFrame(rp_df_data)], ignore_index=True)
        ...
        tabs['Central_Bank_Gold'] = self.create_empty_central_bank_gold_tab()
        tabs['TIC_Flows'] = self.create_empty_tic_flows_tab()
    ...
    def create_empty_central_bank_gold_tab(self):
        """Create empty Central Bank Gold tab"""
        dates = []
        ...
        df_data = {
            'Date': dates,
            'CB_Gold_Purchases_Tonnes': [''] * len(dates),
            'Net_Purchases': [''] * len(dates),
            'Major_Buyers': [''] * len(dates),
            'Data_Source': [''] * len(dates),
            'Notes': [''] * len(dates)
        }
        ...
        instructions = pd.DataFrame({
            'Date': [
                ...
                'MANUAL ENTRY NEEDED',
                'Source: World Gold Council quarterly reports',
                'Frequency: Quarterly',
                'Time Period: 12 quarters (3 years)',
                'Expected Range: -200 to +800 tonnes per quarter',
                'Priority: MEDIUM',
                '--- ENTER DATA BELOW ---'
            ],
            'CB_Gold_Purchases_Tonnes': [''] * 8,
            'Net_Purchases': [''] * 8,
            'Major_Buyers': [''] * 8,
            'Data_Source': [''] * 8,
            'Notes': [''] * 8
    ...
    def create_empty_tic_flows_tab(self):
        """Create empty TIC Flows tab"""
        dates = []
        ...
        df_data = {
            'Date': dates,
            'TIC_Net_Flows_Billions': [''] * len(dates),
            'Foreign_Purchases': [''] * len(dates),
            'Foreign_Sales': [''] * len(dates),
            'Net_Flow': [''] * len(dates),
            'Data_Source': [''] * len(dates),
            'Notes': [''] * len(dates)
        }
        ...
        instructions = pd.DataFrame({
            'Date': [
                'TIC NET FLOWS - Monthly Data',
                'MANUAL ENTRY NEEDED',
                'Source: US Treasury TIC (Treasury International Capital) reports',
                'Frequency: Monthly',
                'Time Period: 24 months',
                'Expected Range: -50 to +100 billion USD monthly',
                'Priority: MEDIUM',
                '--- ENTER DATA BELOW ---'
            ],
            'TIC_Net_Flows_Billions': [''] * 8,
            'Foreign_Purchases': [''] * 8,
            'Foreign_Sales': [''] * 8,
            'Net_Flow': [''] * 8,
            'Data_Source': [''] * 8,
