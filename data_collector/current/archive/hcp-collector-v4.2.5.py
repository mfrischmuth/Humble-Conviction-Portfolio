...
class HCPUnifiedCollector:
    ...
    def _calculate_us_leadership_indicators(self):
        ...
        if 'spy' in self.raw_data and 'efa' in self.raw_data:
            ...
            # Their relative price ratio gives us US weight in developed markets
            # Note: This will be ~85-87% as it excludes emerging markets
            us_pct_current = (spy_current / (spy_current + efa_current)) * 100
            
            # Calculate historical percentages
            min_len = min(len(spy_hist), len(efa_hist))
            pct_history = [(s/(s+e))*100 for s, e in zip(spy_hist[-min_len:], efa_hist[-min_len:])]
            
            self.calculated_indicators['us_market_pct'] = {
                'current_value': us_pct_current,
                'monthly_history': pct_history,
                'source': 'SPY/(SPY+EFA) proxy for developed markets'
            }
