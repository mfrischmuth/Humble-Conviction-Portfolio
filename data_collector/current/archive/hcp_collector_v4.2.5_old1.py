...
- VTI/VT ratio replaces US Market Cap %
...
4. US Leadership: SPY/EFA, VTI/VT, ETF Flows
...
class HCPUnifiedCollector:
    ...
    def _calculate_innovation_indicators(self):
        ...
        # Tech Employment %
        if 'tech_employment' in self.raw_data:
            ...
            logger.info(f"  ✓ Tech Employment: {tech_data['current_value']:.1f}")
    ...
    def _calculate_us_leadership_indicators(self):
        ...
        # VTI/VT Ratio (US market cap %)
        if 'vti' in self.raw_data and 'vt' in self.raw_data:
            vti_hist = self.raw_data['vti']['monthly_history']
            vt_hist = self.raw_data['vt']['monthly_history']
            
            min_len = min(len(vti_hist), len(vt_hist))
            ratio_history = [vti/vt for vti, vt in zip(vti_hist[-min_len:], vt_hist[-min_len:])]
            
            self.calculated_indicators['vti_vt_ratio'] = {
                'current_value': ratio_history[-1] if ratio_history else None,
                'monthly_history': ratio_history,
                'source': 'Yahoo Finance calculation'
            }
            logger.info(f"  ✓ VTI/VT Ratio: {ratio_history[-1]:.3f}")
    ...
    def _assess_data_quality(self) -> Dict:
        ...
        indicators_with_history = {
            ...
            'tech_employment': self.raw_data.get('tech_employment', {}).get('values', []),
            ...
            'vti_vt_ratio': self.calculated_indicators.get('vti_vt_ratio', {}).get('monthly_history', []),
    ...
    def _report_status(self):
        ...
        print("\nIndicator Summary:")
        print("-" * 40)
        for indicator, data in self.calculated_indicators.items():
            if isinstance(data, dict) and 'current_value' in data:
                print(f"  {indicator}: {data['current_value']:.2f}")
