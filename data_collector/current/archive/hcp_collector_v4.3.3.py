"""
HCP Data Collector v4.3.3 - IPS v4.2 Compliant
REAL DATA ONLY - No fallbacks except COFER and accepted proxies
Last Updated: 2025-09-08 01:00:00 UTC
Compatible with: IPS v4.2, FileHandler v2.1, ThemeCalculator v3.2

v4.3.3 Changes:
- Fixed filename output to use standard 'hcp_master_data.json'
- Better preservation of existing COFER data
- Properly sets data_points for all indicators
"""
...
# Configuration
FRED_API_KEY = "82fa4bd8294df4c17d0bde5a37903e57"  # Replace with actual key
...
logger = logging.getLogger('HCP_Collector_v4.3.3')
...
class IPSCompliantCollector:
    ...
    v4.3.3 - Fixed filename and COFER handling
    """
    
    def __init__(self):
        self.master_file = DATA_DIR / "hcp_master_data.json"  # FIXED: Standard filename
        ...
        logger.info("=" * 60)
        logger.info("HCP DATA COLLECTOR v4.3.3 - REAL DATA ONLY")
        logger.info("IPS v4.2 Compliant - Standard filename output")
        logger.info("=" * 60)
    ...
    def collect_cofer_usd(self):
        """
        Check for existing COFER data or provide instructions for extraction
        v4.3.3 - Better handling of existing data
        """
        logger.info("Checking for COFER USD Reserve Share...")
        
        # First check if COFER data already exists in the master file
        if self.master_file.exists():
            try:
                with open(self.master_file, 'r') as f:
                    existing_data = json.load(f)
                
                # Check if COFER exists in indicators section
                if 'indicators' in existing_data and 'cofer_usd' in existing_data['indicators']:
                    cofer = existing_data['indicators']['cofer_usd']
                    # If it has actual data, preserve it
                    if cofer.get('quarterly_history') and len(cofer['quarterly_history']) > 0:
                        self.indicators['cofer_usd'] = cofer
                        # Ensure data_points is set correctly
                        self.indicators['cofer_usd']['data_points'] = len(cofer['quarterly_history'])
                        logger.info(f"  ✓ COFER USD: Preserved existing {len(cofer['quarterly_history'])} quarters")
                        return True
            except:
                pass
        
        # No existing data - provide instructions
    ...
    def collect_put_call_ratio(self):
        ...
        Combines CBOE historical file with manual updates
        """
        ...
        try:
            ...
            # 1. Read CBOE historical file (2006-2019)
            if self.cboe_file.exists():
                ...
                logger.info(f"    Found {len(historical_pc)} months in historical file")
            
            # 2. Check for manual updates file (2019-2025)
            manual_file = HISTORICAL_DIR / "CBOE_manual_updates.csv"
            if manual_file.exists():
                logger.info("  Reading manual CBOE updates...")
                try:
                    manual_df = pd.read_csv(manual_file)
                    # Expected format: Date, Put_Call_Ratio
                    for _, row in manual_df.iterrows():
                        if 'Date' in row and 'Put_Call_Ratio' in row:
                            date_obj = pd.to_datetime(row['Date'])
                            month_key = date_obj.strftime('%Y-%m')
                            
                            if month_key not in historical_pc:
                                historical_pc[month_key] = []
                            historical_pc[month_key].append(float(row['Put_Call_Ratio']))
                    
                    logger.info(f"    Added manual data points")
                except Exception as e:
                    logger.warning(f"    Could not read manual updates: {e}")
            
            if not historical_pc:
                logger.error("  ✗ No Put/Call data found")
                return False
            ...
            # Determine data range for description
            first_year = sorted_months[0][:4]
            last_year = sorted_months[-1][:4]
            
            self.indicators['put_call_ratio'] = {
                'current_value': values[-1],
                'monthly_history': values,
                'monthly_dates': dates,
                'source': f'CBOE Total Put/Call Ratio ({first_year}-{last_year})',
                'last_updated': datetime.now().isoformat(),
                'data_quality': 'real',
                'data_points': len(values),
                'data_note': 'CBOE historical + manual updates if available'
            }
            logger.info(f"  ✓ Put/Call Ratio: {len(values)} months ({first_year}-{last_year})")
            return True
