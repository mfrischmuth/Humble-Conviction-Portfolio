...
Version: 5.1.1
Filename: hcp_collector_v5.1.1.py
Last Updated: 2025-09-09T03:00:00 UTC

CHANGES IN v5.1.1:
- Added real EPS collection from Alpha Vantage (up to 20 years)
- Fetches earnings for AAPL, MSFT, GOOGL, AMZN, NVDA
- Calculates delivery rates (actual/estimate) and averages
- Falls back to existing data if Alpha Vantage fails

MAJOR CHANGES IN v5.1:
- Integrated COFER extraction from IMF CSV files
- Intelligent detection of IMF COFER files in csv_imports
- NaN-safe JSON handling throughout
- Unified data flow - no more separate extractor needed
- All data saves to ./data/hcp_master_data.json
"""
...
class Config:
    """Centralized configuration - v5.1.1"""
...
# ============================================================================
# MAIN COLLECTOR v5.1.1
# ============================================================================

class HCPDataCollectorV5:
    """Data collector v5.1.1 with integrated COFER and EPS extraction"""
    ...
    def collect_cofer(self) -> bool:
        ...
        v5.1: Now checks for IMF CSV files in csv_imports first
        """
    ...
    def collect_put_call_ratio(self) -> bool:
        ...
        # For v5.1, preserve existing recovered data
    ...
    def collect_eps_delivery(self) -> bool:
        ...
        v5.1.1: Real implementation from v4.3.6
        """
        self.logger.info("📈 Collecting EPS Delivery (up to 20 years)...")
        
        # Check for existing good data first
        if 'eps_delivery' in self.indicators:
            existing_points = len(self.indicators['eps_delivery'].get('quarterly_history', []))
            if existing_points > 60:  # 15+ years of data
                self.logger.info(f"  ✓ Preserving {existing_points} quarters of existing EPS data")
                return True
        ...
        try:
            ...
            if not quarterly_delivery:
                self.logger.warning("  ⚠ No EPS data collected from Alpha Vantage")
                # Preserve existing if available
                if 'eps_delivery' in self.indicators:
                    return True
                return False
            ...
            success = self.update_indicator('eps_delivery', new_data)
            if success:
            ...
            # Don't fail if we have existing data
            if 'eps_delivery' in self.indicators:
                return True
            return False
    ...
    def import_csv_updates(self) -> int:
        ...
        v5.1: Now intelligently detects and handles COFER files
        """
        ...
        if cofer_found:
            self.logger.info("  🏦 COFER data successfully integrated")
        
        return imported
    
    # ========================================================================
    # Theme Organization - ALIGNED WITH IPS v4.3.2
    # ========================================================================
    ...
    def organize_into_themes(self):
        ...
        theme_mappings = {
            ...
            'usLeadership': ['spy_efa_momentum', 'us_market_pct', 'etf_flow_differential']
        }
    ...
    def collect_all_indicators(self) -> Dict[str, Any]:
        ...
        self.logger.info(f"COFER Support: INTEGRATED")
        self.logger.info(f"EPS Support: ALPHA VANTAGE (20 years)")
        self.logger.info("=" * 60)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='HCP Data Collector v5.1.1 - With integrated COFER and EPS extraction'
    )
    ...
    if data['metadata'].get('collection_complete', False):
        if collector.save_data(data):
            ...
            print(f"   COFER: INTEGRATED")
            print(f"   EPS: ALPHA VANTAGE (20 years)")
            ...
            print("   - EPS: Alpha Vantage (5 calls/min limit, ~1 min to collect)")
    else:
