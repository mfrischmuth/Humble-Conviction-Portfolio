...
Version: 5.4.0
Filename: hcp_collector_v5.4.0.py
Last Updated: 2025-09-09T22:15:00 UTC
...
class Config:
    """Centralized configuration - v5.4.0"""
...
# ============================================================================
# MAIN COLLECTOR v5.4.0
# ============================================================================

class HCPDataCollectorV5:
    """Data collector v5.4.0 with fixed CAPE handling"""
    ...
    def collect_cape_rate_of_change(self) -> bool:
        """
        Collect CAPE Rate of Change (v5.4.0 - Fixed)
        Handles header rows and date parsing issues
        """
        ...
        try:
            ...
            try:
                ...
                for i, line in enumerate(lines):
                    ...
                    # Common formats: 1871.01, 1871-01, 01/1871, etc.
                    first_field = line.split(',')[0].strip()
                    if first_field and any(char.isdigit() for char in first_field):
                        # Try to parse as a potential date
                        try:
                ...
                date_parsed = False
                for date_format in ['%Y.%m', '%Y-%m', '%Y/%m', '%m/%Y', '%Y-%m-%d', '%m/%d/%Y']:
                    try:
                ...
                if not date_parsed:
                    ...
                    try:
                        ...
                        date_parsed = True
                        self.logger.info(f"  Parsed dates with automatic format detection")
                    except Exception as e:
                        self.logger.error(f"  ✗ Could not parse dates: {e}")
                        self.logger.info(f"  First few date values: {cape_data['Date'].head().tolist()}")
                        return False
                ...
                self.logger.info(f"  Date range: {cape_data.index[0].strftime('%Y-%m')} to {cape_data.index[-1].strftime('%Y-%m')}")
                ...
                date_diffs = cape_data.index.to_series().diff().dt.days
                median_diff = date_diffs.median()
                
                if 25 <= median_diff <= 35:  # Roughly monthly
                    # Data is already monthly
                    self.logger.info(f"  Data is monthly (median gap: {median_diff:.0f} days)")
                else:
                    # Resample to monthly if needed
                    cape_roc = cape_roc.resample('M').last().dropna()
                    self.logger.info(f"  Resampled to monthly frequency")
    ...
    def collect_put_call_ratio(self) -> bool:
        ...
        # For v5.4.0, preserve existing recovered data
    ...
    # ========================================================================
    # DEPRECATED INDICATOR MARKERS (v5.4.0)
    # ========================================================================
    
    def mark_deprecated_indicators(self):
        """Mark indicators as deprecated but preserve their data - Updated for v5.4.0"""
        deprecated = {
    ...
    # ========================================================================
    # Theme Organization - UPDATED FOR v5.4.0
    # ========================================================================
    
    def organize_into_themes(self):
        """Organize flat indicators into themed structure - UPDATED FOR v5.4.0"""
        theme_mappings = {
            'usd': [
                'dxy_index', 
    ...
    # ========================================================================
    # Main Collection Orchestration - UPDATED FOR v5.4.0
    # ========================================================================
    ...
    def collect_all_indicators(self) -> Dict[str, Any]:
        ...
        self.logger.info(f"Major Changes: Fixed CAPE handling, no archiving")
        self.logger.info("=" * 60)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='HCP Data Collector v5.4.0 - Fixed CAPE handling, no archiving'
    )
    ...
    if data['metadata'].get('collection_complete', False):
        if collector.save_data(data):
            ...
            print("   - CAPE: 'CAPE Data.csv' with dates in column A, CAPE in column M")
            print("   - COFER: Download from https://data.imf.org/COFER")
            print("   - Put/Call: CBOE historical data")
            print("\n📝 No archiving in v5.4.0 - files remain for next update")
    else:
