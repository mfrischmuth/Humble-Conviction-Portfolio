...
Version: 5.5.0
Filename: hcp_collector_v5.5.0.py
Last Updated: 2025-09-09T22:30:00 UTC

MAJOR CHANGES IN v5.5.0:
- Fixed CAPE date parsing for YYYY.M format where .1 = October (.10 with trailing zero dropped)
- Now correctly collects full CAPE history (1700+ months)
- Improved date format debugging

MAJOR CHANGES IN v5.4.0:
- Fixed CAPE Data.csv handling (skips header rows automatically)
- Removed CSV archiving - files stay in csv_imports for easy monthly updates
- More robust date parsing for CAPE data
...
class Config:
    """Centralized configuration - v5.5.0"""
...
# ============================================================================
# MAIN COLLECTOR v5.5.0
# ============================================================================

class HCPDataCollectorV5:
    """Data collector v5.5.0 with fixed CAPE date parsing"""
    ...
    def collect_cape_rate_of_change(self) -> bool:
        """
        Collect CAPE Rate of Change (v5.5.0 - Fixed date parsing)
        Handles YYYY.M format where .1 = October (.10 with trailing zero dropped)
        """
        ...
        try:
            ...
            try:
                # First, let's peek at the file to find where the data starts
                ...
                for i, line in enumerate(lines):
                    # Check if line starts with something that looks like a date
                    ...
                    if first_field and any(char.isdigit() for char in first_field):
                        ...
                        if '.' in first_field or '-' in first_field or '/' in first_field:
                            # Likely a date, use this row
                            data_start_row = i
                            first_date_value = first_field
                            self.logger.info(f"  Data starts at row {data_start_row + 1}")
                            self.logger.info(f"  First date value: {first_date_value}")
                            break
                
                # Now read the CSV, skipping header rows
                df = pd.read_csv(cape_file, skiprows=data_start_row, header=None)
                
                # Assign column names based on position
                # Column 0 (A) = Date, Column 12 (M) = CAPE
                if df.shape[1] < 13:
                    self.logger.error(f"  ✗ CSV has only {df.shape[1]} columns, need at least 13 (A-M)")
                    return False
                
                # Extract just the columns we need
                cape_data = pd.DataFrame({
                    'Date': df.iloc[:, 0],  # Column A
                    'CAPE': df.iloc[:, 12]  # Column M
                })
                
                self.logger.info(f"  Loaded {len(cape_data)} rows from CAPE Data.csv")
                
                # Clean the CAPE column - convert to numeric, dropping any non-numeric values
                cape_data['CAPE'] = pd.to_numeric(cape_data['CAPE'], errors='coerce')
                
                # Remove any rows where CAPE is NaN or <= 0 BEFORE parsing dates
                cape_data = cape_data[cape_data['CAPE'].notna() & (cape_data['CAPE'] > 0)]
                self.logger.info(f"  Valid CAPE values: {len(cape_data)} rows")
                
                # Show sample of date values for debugging
                self.logger.info(f"  Sample date values: {cape_data['Date'].head(3).tolist()}")
                
                # Parse dates - try multiple formats
                date_parsed = False
                
                # Special handling for YYYY.M format where .1 = October (.10 with trailing zero dropped)
                if '.' in str(cape_data['Date'].iloc[0]):
                    try:
                        def parse_year_month_decimal(date_str):
                            # Split on period: "2025.09" -> ["2025", "09"] or "2025.1" -> ["2025", "1"]
                            parts = str(date_str).split('.')
                            if len(parts) == 2:
                                year = int(parts[0])
                                month_str = parts[1]
                                
                                # Special case: "1" means October (month 10) because it's .10 with trailing zero dropped
                                if month_str == '1':
                                    month = 10
                                else:
                                    # For all other cases, parse as integer
                                    month = int(month_str)
                                
                                # Validate month
                                if month < 1 or month > 12:
                                    raise ValueError(f"Invalid month {month} from {date_str}")
                        ...
                        date_parsed = True
                        self.logger.info("  Parsed dates from YYYY.M format (where .1 = October)")
                        
                        # Log some examples to verify parsing
                        sample_dates = cape_data.head(5)
                        for idx, row in sample_dates.iterrows():
                            self.logger.debug(f"    {df.iloc[idx, 0]} -> {row['Date'].strftime('%Y-%m')}")
                        
                    except Exception as e:
                        self.logger.error(f"  Failed to parse decimal date format: {e}")
                        pass
                
                # Try other formats if not parsed yet
                if not date_parsed:
                    formats_to_try = [
                        '%Y-%m',      # 1871-01
                        '%Y/%m',      # 1871/01
                    ...
                    for date_format in formats_to_try:
                        try:
                            cape_data['Date'] = pd.to_datetime(cape_data['Date'], format=date_format)
                            date_parsed = True
                            self.logger.info(f"  Parsed dates with format: {date_format}")
                            break
                        except:
                            continue
                ...
                if not date_parsed:
                    ...
                    try:
                        cape_data['Date'] = pd.to_datetime(cape_data['Date'], infer_datetime_format=True)
                        date_parsed = True
                        self.logger.info("  Parsed dates with automatic format detection")
                    except Exception as e:
                        self.logger.error(f"  ✗ Could not parse dates: {e}")
                        self.logger.info(f"  Date samples: {cape_data['Date'].head(10).tolist()}")
                        return False
                
                # Set date as index
                cape_data = cape_data.set_index('Date')
                
                # Sort by date
                cape_data = cape_data.sort_index()
                
                # Verify dates are reasonable
                date_range_start = cape_data.index[0]
                date_range_end = cape_data.index[-1]
                
                self.logger.info(f"  Valid CAPE data: {len(cape_data)} months")
                self.logger.info(f"  Date range: {date_range_start.strftime('%Y-%m')} to {date_range_end.strftime('%Y-%m')}")
                
                # Check if dates make sense (should be historical data)
                if date_range_start.year < 1800 or date_range_start.year > 2025:
                    self.logger.error(f"  ✗ Suspicious start date: {date_range_start}")
                    return False
                ...
                self.logger.info(f"  Calculated RoC: {len(cape_roc)} data points")
                
                # Ensure we have substantial history
                if len(cape_roc) < 100:
                    self.logger.warning(f"  ⚠️ Only {len(cape_roc)} RoC points (expected more)")
                ...
                new_data = {
                    ...
                    'frequency': 'monthly',
                    'garch_suitable': True,
                    'original_cape_range': f"{cape_data['CAPE'].min():.1f} to {cape_data['CAPE'].max():.1f}",
                    'current_cape': round(float(cape_data['CAPE'].iloc[-1]), 2),
                    'date_range': f"{date_range_start.strftime('%Y-%m')} to {date_range_end.strftime('%Y-%m')}"
                }
                ...
                if success:
                    ...
                    self.logger.info(f"    CAPE range: {cape_data['CAPE'].min():.1f} to {cape_data['CAPE'].max():.1f}")
                ...
                self.logger.error(f"  ✗ Error processing CAPE Data.csv: {e}")
                import traceback
    ...
    def collect_put_call_ratio(self) -> bool:
        ...
        # For v5.5.0, preserve existing recovered data
    ...
    # ========================================================================
    # DEPRECATED INDICATOR MARKERS (v5.5.0)
    # ========================================================================
    
    def mark_deprecated_indicators(self):
        """Mark indicators as deprecated but preserve their data - Updated for v5.5.0"""
        deprecated = {
    ...
    # ========================================================================
    # CSV Import Functions - v5.4.0 NO ARCHIVING
    # ========================================================================
    
    def import_csv_updates(self) -> int:
        """
        Import any CSV files in the import directory
        v5.4.0: No archiving, better CAPE handling
        """
        ...
        for csv_file in csv_files:
            ...
            if 'cape' in csv_file.name.lower() and 'data' in csv_file.name.lower():
                cape_found = True
                self.logger.info(f"  📊 Found {csv_file.name} - reserving for CAPE Rate of Change collector")
                # Don't process CAPE here - it will be handled by collect_cape_rate_of_change()
                # v5.4.0: No archiving - file stays in place
                continue
            ...
            if self.csv_importer.detect_imf_cofer_file(csv_file):
                ...
                if new_data:
                    ...
                    if self.update_indicator('cofer_usd', new_data):
                        ...
                        # v5.4.0: No archiving - file stays in place for next month's update
                continue
        ...
        if cape_found:
            self.logger.info("  📈 CAPE Data.csv found - will be processed for CAPE Rate of Change")
        
        return imported
    
    # ========================================================================
    # Theme Organization - UPDATED FOR v5.5.0
    # ========================================================================
    
    def organize_into_themes(self):
        """Organize flat indicators into themed structure - UPDATED FOR v5.5.0"""
        theme_mappings = {
            'usd': [
                'dxy_index', 
    ...
    # ========================================================================
    # Main Collection Orchestration - UPDATED FOR v5.5.0
    # ========================================================================
    ...
    def collect_all_indicators(self) -> Dict[str, Any]:
        ...
        self.logger.info(f"Major Changes: Fixed CAPE date parsing for full history")
        self.logger.info("=" * 60)
        ...
        self.metadata = {
            ...
            'csv_archiving': 'disabled in v5.4.0 for easier monthly updates',
            'cape_date_parsing': 'fixed in v5.5.0 for YYYY.M format'
        }
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='HCP Data Collector v5.5.0 - Fixed CAPE date parsing'
    )
    ...
    if data['metadata'].get('collection_complete', False):
        if collector.save_data(data):
            ...
            print("\nℹ️ CSV files stay in ./data/csv_imports/ for easy monthly updates")
            print("   - CAPE: 'CAPE Data.csv' with dates in column A (YYYY.M format), CAPE in column M")
            print("   - COFER: Download from https://data.imf.org/COFER")
            print("   - Put/Call: CBOE historical data")
            print("\n📝 v5.5.0: Fixed CAPE date parsing where .1 = October (.10 with trailing zero dropped)")
    else:
