...
Version: 5.1.0
Filename: hcp_collector_v5.1.0.py
Last Updated: 2025-09-09T02:00:00 UTC
...
CHANGES FROM v5.0.1:
- Added detect_imf_cofer_file() method
- Added extract_cofer_from_imf() method  
- Enhanced import_csv_updates() to handle COFER files
- Added safe_float_conversion() for NaN handling
- Removed need for separate COFER extractor script
"""
...
class Config:
    """Centralized configuration - v5.1"""
...
class HCPDataCollectorV5:
    """Data collector v5.1 with integrated COFER extraction"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='HCP Data Collector v5.1 - With integrated COFER extraction'
    )
    ...
    if data['metadata'].get('collection_complete', False):
        if collector.save_data(data):
            ...
            print("   - EPS: Earnings delivery data")
    else:
