...
Version: 6.1.0
Filename: hcp_collector_v6.1.0.py
Last Updated: 2025-01-10T17:45:00 UTC
...
class Config:
    """Centralized configuration - v6.1.0"""
...
class SignalTransformer:
    """Handles all indicator transformations for v6.1.0"""
...
# ============================================================================
# MAIN COLLECTOR v6.1.0
# ============================================================================

class HCPDataCollectorV6:
    """Data collector v6.1.0 with signal transformations and fixed P/C merge"""
    ...
    def collect_tic_foreign_demand(self) -> bool:
        ...
        try:
            ...
            if not tic_data:
                ...
                return self.update_indicator('treasury_foreign_demand', new_data)
            ...
            success = self.update_indicator('treasury_foreign_demand', new_data)
            if success:
    ...
    def collect_trailing_pe(self) -> bool:
        """Collect Trailing P/E with deviation from 3-month average"""
        self.logger.info("📈 Collecting Trailing P/E with transformation...")
        
        # First, try to load from existing indicator data (may be from CSV)
        if 'trailing_pe' in self.indicators:
            existing_data = self.indicators['trailing_pe']
            
            # Check if we have monthly history from CSV import
            if 'monthly_history' in existing_data and 'monthly_dates' in existing_data:
                raw_values = existing_data['monthly_history']
                raw_dates = existing_data['monthly_dates']
                
                if len(raw_values) >= 3:
                    self.logger.info(f"  ✓ Found {len(raw_values)} existing P/E data points")
                    
                    # Try to add current value from Yahoo
                    try:
                        spy = yf.Ticker("SPY")
                        info = spy.info
                        ...
                        if current_pe:
                            ...
                            if raw_dates[-1] != current_date:
                                raw_values.append(round(current_pe, 2))
                                raw_dates.append(current_date)
                                self.logger.info(f"  ✓ Added current P/E: {current_pe:.2f}")
                    except Exception as e:
                        self.logger.debug(f"  Could not fetch current P/E: {e}")
                    ...
                    if transformation:
                        new_data = {
                            ...
                            'transformed_dates': transformation['transformed_dates'],
                            'transformation': transformation['transformation'],
                            'source': existing_data.get('source', 'CSV Import'),
                            'last_updated': datetime.now().isoformat(),
                            'data_quality': 'real',
                            'data_points': len(raw_values)
                        }
                        ...
                        if success:
                            ...
                            self.logger.info(f"  ✓ Current P/E: {transformation['current_raw']:.2f}")
                            self.logger.info(f"  ✓ Deviation: {transformation['current_transformed']:.2f}%")
                        return success
            
            # If we have data_points but no arrays, the CSV import didn't create proper structure
            elif existing_data.get('data_points', 0) > 100:
                self.logger.warning(f"  ⚠️ Found {existing_data['data_points']} data points but no historical arrays")
                self.logger.warning(f"  ⚠️ Check CSV format - need 'Date' and 'P/E Ratio' columns")
                # Don't overwrite - preserve what we have
                return True
        
        # If no existing data or insufficient history, try Yahoo for current value only
        try:
            spy = yf.Ticker("SPY")
            info = spy.info
            current_pe = info.get('trailingPE')
            ...
            new_data = {
                ...
                'data_quality': 'real',
                'data_points': 1,
                'note': 'Building history for transformation (need 3+ months)'
            }
            
            success = self.update_indicator('trailing_pe', new_data)
            if success:
                self.logger.info(f"  ✓ P/E: {current_pe:.2f} (transformation pending - need more history)")
            return success
    ...
    # ========================================================================
    # CSV Import and Theme Organization - FIXED P/C MERGE
    # ========================================================================
    
    def import_csv_updates(self) -> int:
        """Import any CSV files in the import directory - FIXED P/C MERGE"""
        csv_files = list(self.config.CSV_IMPORT_DIR.glob("*.csv"))
    ...
    def organize_into_themes(self):
        ...
        theme_mappings = {
            'usd': [
                'dxy_index',
                'treasury_foreign_demand',  # NEW: replaces real_rate_differential
                'cofer_usd'
            ],
            'innovation': [
                'qqq_spy_ratio',
    ...
    def collect_all_indicators(self) -> Dict[str, Any]:
        ...
        self.logger.info(f"Major Feature: P/C Merge Fix + P/E Transform Fix")
        self.logger.info("=" * 60)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='HCP Data Collector v6.1.0 - Fixed P/C Merge and P/E Transform'
    )
    ...
    if data['metadata'].get('collection_complete', False):
        if collector.save_data(data):
            ...
            print("   - P/E: Trailing P/E data (transformed)")
    else:
