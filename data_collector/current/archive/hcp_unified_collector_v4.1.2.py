...
Last Updated: 2025-09-06 21:00:00 UTC
Compatible with: IPS v4.1, File Handler v2.1, Theme Calculator v3.2

SIMPLIFIED DESIGN:
- Single mode: Always collects full historical data (20+ years)
- No persistent storage assumed - complete collection each run
- Outputs to: .../data_collector/Outputs/
- Ready for monthly rebalancing workflow

MAJOR FEATURES:
- Real data sources for all indicators
...
DATA COLLECTION:
- 240 months (20 years) of monthly data for GARCH
- 252 days (1 year) of daily data for MAs
- Current values for real-time tracking

USAGE:
python hcp_unified_collector_v4.1.1.py

No parameters needed - always collects everything required.
"""
    ...
    def __init__(self, mode='monthly'):
        ...
        self.data_config = {
            ...
            'monthly': {
                ...
                'daily_recent': 150,     # 150 days to ensure 6-month momentum calc
                'collect_current': True
            }
        }[mode]
    ...
    def _fetch_fred_data(self):
        ...
        for key, series_id in series_ids.items():
            try:
                ...
                if data:
                    self.raw_data[key] = data
                    logger.info(f"  [OK] {key}: {len(data['values'])} observations")
                else:
                    logger.warning(f"  [WARN] {key}: No data retrieved")
            except Exception as e:
