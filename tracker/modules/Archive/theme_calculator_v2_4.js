/**
 * HCP Portfolio Tracker - Theme Calculator Module v2.4
 * File: theme_calculator_v2_4.js
 * Based on: IPS v3.9 Appendix H Mathematical Specifications
 * Last Updated: 2025-08-30 18:15:00 UTC
 * 
 * CHANGES IN v2.4:
 ...
 * 
 * CHANGES IN v2.3:
 * - Fixed theme naming mismatch: map 'ai' results to 'innovation' theme data
 * - FileHandler v1.2 generates 'innovation' theme, not 'ai' theme
 * 
 * CHANGES IN v2.2:
 * - Fixed nested data structure lookup for FileHandler v1.2 compatibility
 * - Added findIndicatorInThemes helper function
 * - Updated calculateThemeProbability to handle nested theme structure
 * 
 * HANDLES:
 ...
 * 
 * REPLACES: theme_calculator_v2_1.js (direct lookup)
 */
...
const ThemeCalculator = {
    ...
    // IPS v3.9 Indicator Specifications with snake_case keys (PRD v3.3)
    indicatorSpecs: {
        ...
        usd: {
            dxy: {
                ...
                short_ma: 200, // 200-day MA
                long_ma: 400,  // vs 400-day MA
                trigger_condition: 'short_below_long', // USD weakness when DXY 200D < 400D
                tier: 'canary',
                weight: 0.30
            },
            reserves: {
                ...
                threshold: -0.005, // YoY change < -0.5%
                tier: 'structural',
                weight: 0.25
            },
            yuan: {
                ...
                method: 'ma_comparison',
                short_ma: 12, // 12-month MA
                long_ma: 36,  // vs 36-month MA
                trigger_condition: 'short_above_long', // Yuan rising when 12M > 36M
                tier: 'primary',
                weight: 0.25
            },
            gold: {
                ...
                method: 'ma_comparison',
                short_ma: 4,  // 4-quarter MA
                long_ma: 12, // vs 12-quarter MA
                trigger_condition: 'short_above_long', // Purchases rising when 4Q > 12Q
                tier: 'structural',
                weight: 0.20
            }
        },
        
        // AI Productivity Theme (3 indicators) - RENAMED to match FileHandler v1.2
        innovation: {
            productivity: {
                dataKey: 'productivity',
                name: 'Productivity Growth',
                method: 'ma_comparison',
                short_ma: 2,  // 2-quarter MA
                long_ma: 6,  // vs 6-quarter MA
                trigger_condition: 'short_above_long', // Boom when 2Q > 6Q
                tier: 'structural',
                weight: 0.40,
                calibrated: true,
                trigger_rate: 0.477 // 47.7% validated
            },
            qqqSpy: {
                dataKey: 'qqq_spy',
                name: 'QQQ/SPY Ratio',
                method: 'ma_comparison',
                short_ma: 50,  // 50-day MA
                long_ma: 200, // vs 200-day MA
                trigger_condition: 'short_above_long', // Tech leadership when 50D > 200D
                tier: 'canary',
                weight: 0.35
            },
            margins: {
                ...
                ma_period: 36, // 3-year MA
                threshold_offset: 0.005, // TTM > 3Y MA + 0.5%
                tier: 'primary',
                weight: 0.25
            }
        },
        ...
        pe: {
            forwardPE: {
                ...
                name: 'Forward P/E',
                method: 'ma_comparison',
                short_ma: 12, // 1-year MA
                long_ma: 36,  // vs 3-year MA
                trigger_condition: 'short_above_long', // Overvaluation when 1Y > 3Y
                tier: 'primary',
                weight: 0.40,
                calibrated: true,
                trigger_rate: 0.494 // 49.4% validated
            },
            cape: {
                dataKey: 'cape',
                name: 'Shiller CAPE',
                method: 'vs_long_ma',
                ma_period: 240, // 20-year MA
                trigger_condition: 'current_above_ma', // High when current > 20Y MA
                tier: 'primary',
                weight: 0.35
            },
            riskPremium: {
                ...
                method: 'ma_comparison',
                short_ma: 6,  // 6-month MA
                long_ma: 18, // vs 18-month MA
                trigger_condition: 'short_below_long', // Compressed when 6M < 18M
                tier: 'canary',
                weight: 0.25
            }
        },
        
        // International Theme (4 indicators - updated keys)
        intl: {
            acwxSpy: {
                dataKey: 'acwx_spy',
                name: 'ACWX/SPY Relative',
                method: 'ma_comparison',
                short_ma: 30, // 30-day MA
                long_ma: 90, // vs 90-day MA
                trigger_condition: 'short_above_long', // INTL momentum when 30D > 90D
                tier: 'canary',
                weight: 0.30
            },
            spWorld: {
                ...
                period: 6, // 6-month relative performance
                threshold: -0.02, // US underperforming by >2%
                tier: 'primary',
                weight: 0.35
            },
            usPercent: {
                ...
                short_ma: 12, // 12-month MA
                long_ma: 36, // vs 36-month MA
                trigger_condition: 'short_below_long', // US declining when 12M < 36M
                tier: 'structural',
                weight: 0.20
            },
            ticFlows: {
                ...
                name: 'TIC Net Flows',
                method: 'fixed_threshold',
                threshold: 0, // 12-month sum < 0 (outflows)
                tier: 'structural',
                weight: 0.15
            }
        }
    ...
    /**
     * NEW v2.2: Find indicator in nested theme structure
     * Handles FileHandler v1.2 nested data format
     */
    findIndicatorInThemes: function(dataIndicators, dataKey) {
        // Search through all theme objects to find the indicator
    ...
    getCurrentState: function(indicator, spec) {
        ...
        switch (spec.method) {
            ...
            case 'relative_performance':
                // Implementation depends on specific indicator data structure
    ...
    /**
     * UPDATED v2.2: Calculate theme probability using IPS v3.9 aggregation
     * Now handles nested data structure from FileHandler v1.2
     */
    calculateThemeProbability: function(dataIndicators, themeName) {
        const indicatorSpecs = this.indicatorSpecs[themeName];
        if (!indicatorSpecs) return 0.15;
        ...
        Object.keys(indicatorSpecs).forEach(key => {
            ...
            // FIXED v2.2: Use nested data lookup instead of direct access
    ...
    calculateThemeAnalysis: function(monthlyData, indicators) {
        ...
        // FIXED v2.3: Map theme names to match FileHandler v1.2 output
        const themes = {
            usd: this.calculateThemeProbability(dataIndicators, 'usd'),
            ai: this.calculateThemeProbability(dataIndicators, 'innovation'), // FileHandler uses 'innovation', not 'ai'
    ...
    /**
     * Display enhanced results with IPS v3.9 formatting - PRD v3.3 COMPLIANT
     */
