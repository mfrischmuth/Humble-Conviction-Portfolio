/**
 * HCP Portfolio Tracker - Theme Calculator Module v2.7
 * File: theme_calculator_v2_7.js
 * Based on: IPS v3.10 Appendix H Mathematical Specifications
 * Last Updated: 2025-08-31 21:30:00 UTC
 * 
 * CHANGES IN v2.7:
 * - FULL PRD v3.4 COMPLIANCE: Corrected scenario probability thresholds
 * - ENHANCED VALIDATION: Added comprehensive IPS v3.10 validation checks
 * - IMPROVED ERROR HANDLING: Better missing data detection and reporting
 * - OPTIMIZED CALCULATIONS: Streamlined three-component probability model
 * 
 * MAINTAINED FROM v2.6:
 * - Boundary conditions removed (enables 70-85% tech boom probabilities)
 * - Complete FileHandler v1.4 data alignment with snake_case keys
 * - Nested theme structure navigation
 * - IPS v3.10 three-component methodology
 * 
 * HANDLES:
 * - Enhanced transition probability calculations without boundary overrides
 * - Physics-based time-to-trigger estimation with momentum analysis
 * - Theme aggregation with tier-based weighting (Canary/Primary/Structural)
 * - 16-scenario generation with proper probability ranking
 * - PRD v3.3 compliant display formatting
 * 
 * REPLACES: theme_calculator_v2_6.js
 */

const ThemeCalculator = {
    version: '2.7',
    framework: 'IPS v3.10 Appendix H',
    lastUpdated: '2025-08-31T21:30:00.000Z',
     ...
     * v2.7: Improved error handling and validation
     */
    calculateEnhancedTransitionProbability: function(indicator, spec) {
        try {
            ...
            // Step 5: Time-based probability decay (IPS v3.10 formula)
            let baseProbability;
            ...
            // Step 6: Direction adjustment (IPS v3.10 specification)
            ...
            if (movingTowardTrigger) {
                ...
                // Moving away from trigger - 70% reduction per IPS v3.10
                finalProbability = baseProbability * 0.3;
            }
            
            // Step 7: Apply IPS v3.10 bounds [0.05, 0.95] ONLY
            // NOTE: Boundary conditions from v2.5 and earlier have been REMOVED
            // This allows proper expression of strong signals (70-85% for tech boom)
            const result = Math.min(0.95, Math.max(0.05, finalProbability));
            
            // Debug logging for calibration
            if (spec.calibrated) {
                console.log(`${spec.name}: current=${indicator.current.toFixed(2)}, momentum=${momentum.toFixed(2)}, distance=${(distanceToTrigger*100).toFixed(1)}%, months=${monthsToTrigger.toFixed(1)}, prob=${(result*100).toFixed(1)}%`);
            }
    ...
    /**
     * Determine current state relative to trigger condition
     */
    ...
    /**
     * Calculate momentum using IPS v3.9 methodology
     */
    ...
    calculateThemeAnalysis: function(monthlyData, indicators) {
        ...
        console.log('=== Theme Analysis v2.7 Complete ===');
    ...
    /**
     * PRD v3.3 Compliant Display Method
     */
    displayThemeResults: function(analysis, containerId) {
        ...
        // PRD v3.3 Compliant Theme Display (NO CONFIDENCE LABELS)
        const themeNames = {
        ...
        html += `
                ...
                <div class="scenario-matrix">
                    <h4>Scenario Probability Matrix (Top 8)</h4>
                    <div class="scenario-grid">
        `;
        
        // Display top 8 scenarios with PRD v3.3 color coding
        ...
        console.log('Theme results displayed successfully with PRD v3.3 compliance');
    },

    /**
     * PRD v3.3 Compliant Scenario Color Classification
     */
    getScenarioColorClass: function(probability) {
        // PRD v3.3 5-tier color system
