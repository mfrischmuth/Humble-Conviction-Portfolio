/**
 * HCP Portfolio Tracker - Theme Calculator Module v2.8
 * File: theme_calculator_v2_8.js
 * Based on: IPS v3.10 Appendix H Mathematical Specifications
 * Last Updated: 2025-08-31 22:00:00 UTC
 * 
 * CRITICAL FIX IN v2.8:
 * - CORRECTED TIME-TO-TRIGGER LOGIC: Fixed backward calculation for already-triggered indicators
 * - SIGNAL STRENGTH APPROACH: For triggered indicators, calculate strength not reversal time
 * - MOMENTUM AMPLIFICATION: Strong positive momentum now properly increases probabilities
 * - REALISTIC TECH BOOM RESULTS: Should now produce 70-85% for strong bullish indicators
 * 
...
const ThemeCalculator = {
    ...
    lastUpdated: '2025-08-31T22:00:00.000Z',
    ...
    /**
     * NEW v2.8: Calculate signal strength for already-triggered indicators
     */
    calculateSignalStrength: function(distanceToTrigger, momentum, spec) {
        ...
        // Base probability starts high for already-triggered states
        let baseProbability = 0.60; // Default for triggered state
        
        // Distance from trigger line (how far we are from losing the signal)
        const distanceBonus = Math.min(0.25, Math.abs(distanceToTrigger) * 2); // Up to 25% bonus
        
        // Momentum boost for favorable direction
        let momentumBoost = 0;
        ...
        if (this.isMomentumFavorable(momentum, distanceToTrigger, true)) {
            momentumBoost = Math.abs(momentum) * 0.20; // Up to 20% boost
        } else {
            momentumBoost = -Math.abs(momentum) * 0.15; // Penalty for unfavorable momentum
        }
        
        return Math.max(0.15, Math.min(0.90, baseProbability + distanceBonus + momentumBoost));
    },
    ...
    calculateThemeAnalysis: function(monthlyData, indicators) {
        ...
        console.log('=== Theme Analysis v2.8 Starting ===');
    ...
    displayThemeResults: function(analysis, containerId) {
        ...
        // Display top 8 scenarios with PRD v3.4 color coding
