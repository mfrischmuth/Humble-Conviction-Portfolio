/**
 * HCP Portfolio Tracker - Theme Calculator Module v4.2
 * Based on: IPS v4.2 Practical Implementation with Trending Indicators
 * Last Updated: 2025-09-09 22:30:00 UTC
 * 
 * CHANGES IN v4.2:
 * - Integrated trending indicator analysis for COFER USD
 * - COFER now uses trend acceleration/deceleration instead of GARCH
 * - All other indicators continue using GARCH/percentile analysis
 * - ZERO REGRESSION - All v4.1 functionality preserved
 * 
 * PRESERVED FROM v4.1:
 * - Fixed boundedProjection undefined reference error
 * - Proper variable declaration in calculateTransitionProbabilities
 * - All v4.0 GARCH functionality for cyclical indicators
...
const ThemeCalculator = {
    version: '4.2',
    framework: 'IPS v4.2 with Trending Indicators',
    lastUpdated: '2025-09-09T22:30:00Z',
    ...
    // ===== TRENDING INDICATOR CONFIGURATION =====
    trendingIndicators: {
        cofer_usd: {
            shortTermMonths: 6,   // Recent trend (2 quarterly updates)
            longTermMonths: 60,   // Structural trend (5 years)
            type: 'structural_decline',  // Expecting long-term decline
            inverted: true  // Deceleration of decline = positive signal
        }
        // Future trending indicators can be added here
    },
    ...
    indicators: {
        usd: {
            ...
            lagging: { 
                ...
                minHistory: 26,
                analysisType: 'trending'  // NEW: Mark as trending indicator
            }
        },
        innovation: {
            leading: { 
    ...
    // ===== NEW: TRENDING ANALYSIS METHODS =====
    
    /**
     * Calculate trending indicator signal
     * Returns value between -1 and 1
     */
    calculateTrendingSignal: function(indicatorName, history) {
        const config = this.trendingIndicators[indicatorName];
        if (!config) {
            throw new Error(`${indicatorName} not configured as trending indicator`);
        }
        
        if (!history || history.length < config.longTermMonths) {
            console.warn(`${indicatorName}: Insufficient history for trend analysis`);
            return 0; // Neutral if insufficient data
        }
        
        // Extract windows
        const shortTermData = history.slice(-config.shortTermMonths);
        const longTermData = history.slice(-config.longTermMonths);
        
        // Calculate annualized slopes
        const shortSlope = this.calculateAnnualizedSlope(shortTermData);
        const longSlope = this.calculateAnnualizedSlope(longTermData);
        
        // Acceleration = short-term slope minus long-term slope
        const acceleration = shortSlope - longSlope;
        
        // Transform to [-1, 1] using tanh
        // No artificial dampening - full signal strength
        let signal = Math.tanh(acceleration / 0.01);
        
        // Apply inversion if configured (e.g., for COFER)
        if (config.inverted) {
            signal = -signal;
        }
        
        console.log(`${indicatorName} Trend Analysis:
            Long-term slope: ${(longSlope * 100).toFixed(3)}%/year
            Short-term slope: ${(shortSlope * 100).toFixed(3)}%/year
            Acceleration: ${(acceleration * 100).toFixed(3)}%/year
    ...
    /**
     * Calculate annualized slope from data points
     */
    calculateAnnualizedSlope: function(data) {
        const n = data.length;
        if (n < 2) return 0;
        ...
        // Monthly slope
        const monthlySlope = (n * sumXY - sumX * sumY) / (n * sumXX - sumX * sumX);
        
        // Convert to annualized
        return monthlySlope * 12;
    },
    
    // ===== MAIN CALCULATION FUNCTIONS =====
    
    /**
     * Calculate theme transitions with pure GARCH (no floors) and trending analysis
     */
    ...
    calculateTransitionProbabilities: function(currentValue, trend, volatility) {
        ...
        // Bound projection - FIXED in v4.1: Properly declare the variable
     ...
     * ENHANCED: Now handles trending indicators
     */
    calculateThemeValue: function(theme) {
        ...
        ['leading', 'concurrent', 'lagging'].forEach(temporal => {
            if (theme[temporal]) {
                ...
                // Check if this is a trending indicator
                if (this.trendingIndicators[indicatorName] && indicator.history) {
                    // Use trending analysis
                    const trendingSignal = this.calculateTrendingSignal(
                        indicatorName, 
                        indicator.history
                    );
                    weightedSum += trendingSignal * weight;
                    totalWeight += weight;
                    
                    // Store the trending signal for display
                    indicator.trendingSignal = trendingSignal;
                } else {
                    // Use standard value
    ...
    /**
     * Extract historical data from File Handler v3.1 structure
     * ENHANCED: Handles trending indicators differently
     */
        ...
        for (let i = 0; i < maxLength; i++) {
            ...
            temporals.forEach(temporal => {
                if (theme[temporal] && theme[temporal].history && 
                    ...
                    const weight = this.getTemporalWeight(temporal);
                    const indicatorName = theme[temporal].name;
                    
                    // For trending indicators, use the raw history
                    // For cyclical indicators, use the normalized value
                    if (this.trendingIndicators[indicatorName]) {
                        // Don't include trending indicators in GARCH analysis
    ...
    validateConfiguration: function() {
        console.log("ThemeCalculator v4.2 Configuration Validation");
        console.log("=" + "=".repeat(50));
        
        const checks = {
            "IPS Version": "v4.2 with Trending Indicators (v4.3 preview)",
            "Calibrations": "NONE - Pure GARCH for cyclical indicators",
            "Trending Analysis": "COFER USD uses trend acceleration",
            ...
            "v4.2 Enhancement": "COFER trending analysis integrated"
        };
        ...
        console.log("  ✓ Pure GARCH volatility for cyclical indicators");
        console.log("  ✓ Trend acceleration analysis for COFER USD");
        ...
        console.log("  ✓ ZERO REGRESSION - All v4.1 functionality preserved");
...
if (typeof window !== 'undefined') {
    ...
    console.log("ThemeCalculator v4.2 loaded - IPS v4.2 with Trending Indicators");
}